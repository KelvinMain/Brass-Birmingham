...
export type AiAgentFactory = (
  game: GameState,
  playerId: string,
  random: () => number,
) => AiAgent

export type RunAiTurnOptions = {
  random?: () => number
  createAgent?: AiAgentFactory
  passTurnOptions?: PassTurnOptions
}
...
export { getLoanIncomeAfterReduction } from './ai/estimators'
...
export function runAiTurn(
  game: GameState,
  options: RunAiTurnOptions | (() => number) = {},
): AiTurnResult {
  const resolvedOptions: RunAiTurnOptions =
    typeof options === 'function' ? { random: options } : options
  const random = resolvedOptions.random ?? Math.random
  const createAgent = resolvedOptions.createAgent ?? createStrategicAiAgent
  ...
  for (let actionIndex = 0; actionIndex < actionCount; actionIndex += 1) {
    ...
    const agent = createAgent(currentGame, activePlayer.id, random)
  ...
  while (true) {
    ...
    const discardAgent = createAgent(readyToPass, activePlayer.id, random)
  ...
  const passedGame = applyGameAction(readyToPass, {
    ...
    passTurnOptions: resolvedOptions.passTurnOptions,
  })
